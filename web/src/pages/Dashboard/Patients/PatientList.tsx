  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        ...
        // Show onboarding tooltip if it's a new patient
    ...
    <div className="animate-fade-in">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem', flexWrap: 'wrap', gap: '1rem' }}>
        <div style={{ flex: '1 1 200px' }}>
          <h1 style={{ fontSize: '1.5rem', fontWeight: 800 }}>Patients</h1>
          <p style={{ color: 'var(--muted)', fontSize: '0.9rem' }}>Manage your patient directory</p>
        </div>
        <div style={{ display: 'flex', gap: '1rem', alignItems: 'center', flexWrap: 'wrap', width: '100%', maxWidth: '600px', flex: '2 1 300px' }}>
          <div className="input-group" style={{ marginBottom: 0, flex: 1, minWidth: '200px', position: 'relative' }}>
          ...
          <button onClick={handleOpenAddModal} className="btn btn-primary" style={{ whiteSpace: 'nowrap' }}>
                    ...
                    <td style={{ padding: '1rem', textAlign: 'right' }}>
                      <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'flex-end', alignItems: 'center' }}>
                        ...
                        <div style={{ position: 'relative' }}>
                          <Link to={`/dashboard/patients/${p.id}`} className="hidden-mobile btn btn-outline" style={{ padding: '0.4rem 1rem', fontSize: '0.85rem' }}>View Details</Link>
                          {index === 0 && showOnboardingTooltip && (
                            <Tooltip 
                              isVisible={showOnboardingTooltip} 
                              onClose={() => setShowOnboardingTooltip(false)} 
                              message="Success! Now click 'View Details' to assign a prescription to this patient."
                              position="bottom"
                            />
                          )}
                        </div>
